...
document.addEventListener('DOMContentLoaded', () => { //для того, чтобы скрипт загрузился только после того как загрузится DOM-дерево используется "DOMContentLoaded"
    ...
    addForm.addEventListener('submit', (event) => {
        ...
        const newFilm = addInput.value; //создаем новую переменную - фильм, введенный пользователем
        ...
        movieDB.movies.sort(); //сортируем фильмы из массива по алфавиту
    });
    ...
    createMovieList(movieDB.movies, moveList); //вызываем нашу функцию
});